...
!function (i, d) {
    ...
    b.pt.tips = function () {
        var q = this, p = q.config, r = q.layero;
        var m = [r.outerWidth(), r.outerHeight()], n = f(p.follow);
        if (!n[0]) {
            n = f("body")
        }
        var t = {width: n.outerWidth(), height: n.outerHeight(), top: n.offset().top, left: n.offset().left},
        ...
        r.find("." + c[5]).css({"background-color": p.tips[1], "padding-right": (p.closeBtn ? "30px" : "")});
        r.css({left: t.tipLeft - (p.fixed ? h.scrollLeft() : 0), top: t.tipTop - (p.fixed ? h.scrollTop() : 0)})
    };
    b.pt.move = function () {
            ...
            n.index = g.open(f.extend({
                ...
                area: function () {
                    ...
                    if (x.width < size && x.height < size) {
                        if ((size - x.width) > (size - x.height)) {
                            x.width = (size / x.height) * x.width;
                            x.height = size;
                        }
                        else {
                            x.height = (size / x.width) * x.height;
                            x.width = size;
                        }
                    }
