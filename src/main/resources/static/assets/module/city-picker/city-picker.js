...
(function (factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        layui.define('jquery', function (exports) {
            ...
            var newChineseDistricts = {};
            newChineseDistricts[86] = ChineseDistricts[86];
            $.post({
                url: '/common/listDictItem',
                data: {dictId: "sys.area"},
                success: function (res) {
                    if (res.success) {
                        for (var i = 0; i < res.result.length; i++) {
                            if (res.result[i].parentValue != "") {
                                if (newChineseDistricts.hasOwnProperty(res.result[i].parentValue)) {
                                    newChineseDistricts[res.result[i].parentValue][res.result[i].dictValue] = res.result[i].dictText;
                                }
                                else {
                                    newChineseDistricts[res.result[i].parentValue] = {};
                                    newChineseDistricts[res.result[i].parentValue][res.result[i].dictValue] = res.result[i].dictText;
                                }
                            }
                        }
                        ChineseDistricts = newChineseDistricts;
    ...
    $.fn.citypicker.getCodeByDocument = function (d) {
        var codeArr = [];
        $(d).find('.select-item')
            .each(function () {
                var code = $(this).data('code');
    ...
    // 根据省编码获取该省下的所有市
    $.fn.citypicker.getCityByProvince = function (province_code) {
        var cityObj = ChineseDistricts[province_code];
        var citys = Object.values(cityObj);
        var cityCodes = Object.keys(cityObj);
        var result = [];
        for (var i = 0; i < citys.length; i++) {
            result.push({cityCode: cityCodes[i], city: citys[i]});
        }
        return result;
    }
